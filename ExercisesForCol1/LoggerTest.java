...
class LogSystem<T extends ILog> {
        ...
        LogProcessor<T> firstLogProcessor = (elements)->{

            return elements.stream().filter(element -> element.getType().equals("INFO"))
                    .collect(Collectors.toCollection(ArrayList::new));
        };

        LogProcessor<T> secondLogProcessor = (elements)->{
            return elements.stream().filter(element -> element.getMessage().length()<100)
                    .collect(Collectors.toCollection(ArrayList::new));
        };

        LogProcessor<T> thirdLogProcessor = (elements)->{
            return elements.stream().sorted(Comparator.comparing(ILog::getTimestamp))
                    .collect(Collectors.toCollection(ArrayList::new));
        };
