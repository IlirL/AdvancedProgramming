...
class Schedulers {
    public static <T extends Task> TaskScheduler<T> getOrdered() {
        ...
        TaskScheduler<T> ts = (tasks)->{
            return Arrays.stream(tasks).sorted(Comparator.comparing(Task::getOrder)).collect(Collectors.toCollection(ArrayList::new));
        };
        return ts;
    }
    ...
    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        ...
        TaskScheduler<T> ts = (tasks)->{
            return Arrays.stream(tasks).filter(task -> task.getOrder()<order).collect(Collectors.toCollection(ArrayList::new));
        };
